...
const SparklingStar = ({ children, color = "#FFC700" }) => {
  ...
  const sparklingRef = useRef(null);

  const sparkling = function() {
    const sparklingElement = sparklingRef.current;
    if (!sparklingElement) return;
    ...
    const rand = Math.round(Math.random() * 700) + 100;
  ...
  return (
    <div ref={sparklingRef} className='sparkling'>
      {children}
    </div>
  )
}
